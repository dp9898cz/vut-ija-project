...
public class Vehicle implements Drawable, TimerMapUpdate {
    ...
    @JsonIgnore
    private boolean onAlternativeRoute = false;

    public boolean isOnAlternativeRoute() {
        return onAlternativeRoute;
    }

    public void setOnAlternativeRoute(boolean onAlternativeRoute) {
        this.onAlternativeRoute = onAlternativeRoute;
    }

    public Stop nextStop() {
        int index = -1;
        for (Stop i : path.getStopList()) {
            if (i.getCoordinates().equals(lastStop)) {
                index = path.getStopList().lastIndexOf(i);
            }
        }
        //return path.getStopList().get(++index);
        System.out.println("RETURNING: ");
        System.out.print(path.getStopList().get(index + 1).getName());
        return path.getStopList().get(++index);
    }
